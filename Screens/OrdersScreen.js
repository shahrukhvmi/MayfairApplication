...
const OrdersScreen = () => {
  ...
  /* ─────────────── fetch orders ────────────── */
  const getOrdersMutation = useMutation(getOrders, {
    onSuccess: (res) => {
  ...
  const pillStyle = (s) => {
    switch (s.toLowerCase()) {
      case 'processing':  return [styles.pill, styles.processing];
      case 'incomplete':  return [styles.pill, styles.incomplete];
      case 'approved':    return [styles.pill, styles.approved];
      case 'cancelled':   return [styles.pill, styles.cancelled];
      default:            return [styles.pill, styles.defaultPill];
    }
  };
  ...
  const renderRow = ({ item }) => {
    ...
    return (
      <View style={styles.row}>
        ...
        <TouchableOpacity style={[styles.cell, styles.eye]}
...
const styles = StyleSheet.create({
  ...
  searchInput:   {
    ...
    paddingVertical: 8, paddingLeft: 34, paddingRight: 10,
  ...
  /* header row */
  headerRow:     { flexDirection: 'row', paddingVertical: 8, borderBottomWidth: 1, borderColor: '#e5e7eb' },
  hCell:         { fontSize: 11, fontWeight: '600', color: '#6b7280' },

  /* data rows */
  row:           { flexDirection: 'row', paddingVertical: 14, borderBottomWidth: 1, borderColor: '#e5e7eb' },
